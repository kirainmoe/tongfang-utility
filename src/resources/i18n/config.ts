const configPageTranslation = {
  ...
  CONFIG_PAGE_DESCRIPTION: [
    '管理，更新和定制 OpenCore 配置文件。',
    'Manage, update and customize OpenCore config.',
  ],
  ...
  CONFIG_UTILITY_VERSION_NOT_SATISFIED: [
    'Tongfang Utility 版本不满足要求',
    'Tongfang Utility version is not satisfied',
  ],
  ...
  CONFIG_BUILD_FILE_HASH_NOT_MATCH: [
    '警告：EFI 构建文件 build.yml 哈希值不一致！',
    'Warning: build.yml hash check failed!',
  ],
  ...
  CONFIG_THIS_IS_A_LOCAL_BUILD: [
    '这是一个从本地读取的文件，无法获取更新内容。',
    'This is a local file. We could not know anything :(',
  ],
  CONFIG_INTEL_VERSION_NOT_SATISFIED_CONTENT: [
    '选择的 EFI 版本 (:efi_version) 需要 :driver_type 版本 >= :target_version，你正在使用的版本是 :current_version. 需要先更新 Intel 驱动版本才能下载此版本的 EFI.',
    'Current selected EFI version (:efi_version) required :driver_type version >= :target_version, while you are having :current_version. You need to upgrade Intel drivers version to  download this EFI version.'
  ],
  CONFIG_INTEL_VERSION_NOT_SATISFIED: [
    'Intel 网卡驱动版本不满足要求',
    'Intel wireless driver version is not satisfied',
  ],

  WELCOME_CHECK_COMPONENT_VERSION: ['检查组件版本', 'Component Version'],
  WELCOME_CURRENT_EFI_VERSION: ['当前引导 EFI 版本', 'Current EFI Version'],
  WELCOME_INTEL_WIFI_VERSION: [
    'Intel Wi-Fi 驱动版本',
    'Intel Wi-Fi Driver Version',
  ],
  WELCOME_INTEL_BLUETOOTH_VERSION: [
    'Intel 蓝牙驱动版本',
  ...
  SELECT_MODEL_FILTER_INPUT: [
    '搜索机型 / 模具 / 制造商...',
    'Search model / barebone / vendor...',
  ],
  ...
  CUSTOMIZE_SELECT_OS_VERSION: [
    '选择你要安装的 macOS 版本',
    'Select macOS Version You Want to Install',
  ],
  CUSTOMIZE_SELECT_WIRELESS_ADAPTER_TYPE: [
    '选择无线网卡型号 / 联网方式',
    'Select Wireless Adapter Type',
  ],
  CUSTOMIZE_SELECT_INTERNAL_MONITOR_TYPE: [
    '选择笔记本内屏类型',
    'Select Internal Monitor Type',
  ],
  CUSTOMIZE_SPECIAL_HARDWARES: [
    '硬件兼容设置',
    'Hardwares Compatibility Setting',
  ],
  CUSTOMIZE_SPECIAL_SETTINGS: ['特殊设置', 'Special Setting'],
  CUSTOMIZE_WIRELESS_APPLE: ['Mac 原装无线网卡', 'Mac original'],
  CUSTOMIZE_WIRELESS_BROADCOM: [
    '博通 / 戴尔兼容无线网卡',
    'Broadcom / DELL compatible',
  ],
  CUSTOMIZE_WIRELESS_INTEL: ['Intel 无线网卡', 'Intel wireless'],
  CUSTOMIZE_ANDROID_TETHERING: [
    'Android USB 共享网络',
  ...
  CUSTOMIZE_DESC_1080P: [
    '若你的笔记本内屏分辨率为 1920x1080@60Hz 请选择此项，此选项适合大部分用户。',
    'This option is preferred for normal users.',
  ],
  CUSTOMIZE_DESC_1080P144: [
    '若你的笔记本内屏分辨率为 1920x1080@144Hz 请选择此项。',
    'This option is preferred for those laptops having high refresh rate monitor.',
  ],
  ...
  CUSTOMIZE_DISABLE_NVME: [
    '屏蔽不兼容的 NVMe 硬盘',
    'Disable incompatible NVMe SSD',
  ],
  ...
  CUSTOMIZE_DONT_USE_AIRPORTITLWM: [
    '不使用原生 Intel 驱动',
    'Use itlwm instead of AirportItlwm',
  ],
  ...
  PERSONALIZE_SMBIOS_RANDOM_GENERATE_USING_THIS_MODEL: [
    '随机生成 SMBIOS 信息',
    'Randomly generate SMBIOS',
  ],
  PERSONALIZE_SMBIOS_READ_FROM_SYSTEM: [
    '读取当前系统 SMBIOS 信息 (仅macOS)',
    'Read SMBIOS from system (macOS only)',
  ],
  ...
  PERSONALIZE_SMBIOS_CURRENT_SOURCE: [
    '当前 SMBIOS 来源',
    'Current SMBIOS source',
  ],
  PERSONALIZE_SMBIOS_READ_FROM_CURRENT_SYSTEM: [
    '从当前系统读取',
    'Read from system',
  ],
  PERSONALIZE_SMBIOS_AUTO_GENERATED: ['随机生成', 'Randomly generated'],
  PERSONALIZE_SMBIOS_MANUALLY_MODIFIED: ['手动修改', 'Manual'],
  PERSONALIZE_SMBIOS_LAST_GENERATED: [
    '继承上次生成结果',
    'Inherit last generated',
  ],
  PERSONALIZE_CUSTOM_BACKGROUND: [
    '自定义引导背景',
    'Custom Bootloader Background Image',
  ],
  ...
  PERSONALIZE_DOWNLOAD_FROM: [
    '从 :downloadSourceName 下载',
    'Download from :downloadSourceName',
  ],
  PERSONALIZE_SET_BOOT_ARGS: ['自定义引导参数', 'Custom Boot Args'],
  PERSONALIZE_SET_BOOT_ARGS_PLACEHOLDER: [
    '自定义 macOS 引导参数。如果你不知道这是什么，留空即可；这是一个高级选项，请不要乱选乱填。',
    "Leave it blank if you don't know what is this.",
  ],
  PERSONALIZE_ONLY_SMUUID_ON_WINDOWS: [
    '提示：Windows 环境仅能够获取 SMUUID，其它参数仍为随机生成或上次使用的值。',
    'Note: Only SMUUID will be read on Windows.',
  ],
  ...
  PERSONALIZE_SMBIOS_SHOULD_NOT_BE_EMPTY: [
    'SMBIOS 信息不允许为空，请重新生成或填写有效的 SMBIOS 信息。',
    'SMBIOS fields should not be empty.',
  ],

  GENERATE_ENSURE_OPTIONS: ['确认你的选择', 'Verify Your Choice'],
  GENERATE_CHOSEN_MODEL: ['你选择的机型（模具）：', 'Model (Barebone): '],
  GENERATE_MACOS_VERSION: [
    '你将安装的 macOS 版本：',
    'Version of macOS to install: ',
  ],
  GENERATE_NETWORK_METHOD: ['你选择的联网方式：', 'Wireless adapter vendor: '],
  GENERATE_USE_TETHERING: [
    '，同时使用 Android USB 共享网络',
    ', with Android USB tethering support',
  ],
  GENERATE_MONITOR_TYPE: ['你的内置显示器类型：', 'Internal monitor type: '],
  GENERATE_NVME_DISABLED: [
    '屏蔽了位于第 :slot 个 m.2 插槽的 NVMe 硬盘',
    'Disabled NVMe SSD in slot :slot',
  ],
  GENERATE_NVMEFIDX_ENABLED: ['启用了 NVMe 电源优化', 'Enabled NVMeFix'],
  GENERATE_LOAD_APPLE_GUC: [
    '加载了核显的 Apple GuC 固件',
    'Loaded Apple GuC firmware for IGPU',
  ],
  GENERATE_CPU_BEST_PERFORMANCE: [
    'CPU 模式设置为最佳性能模式',
    'Enabled CPU best performace mode',
  ],
  GENERATE_CUSTOM_BOOT_ARGS: [
    '将添加以下自定义引导参数到 config.plist: ',
    'Will add the following boot args into config.plist',
  ],
  ...
  GENERATE_CANNOT_NEXT_DESCRIPTION: [
    '在进行下一步之前，请先解决以下问题：',
    'Please resolve the following issues before continue.',
  ],
  GENERATE_INTEL_DRIVER_GO_MANAGE: ['前往下载与设置', 'go to settings'],

  PROCESSING_STEP_DOWNLOAD: [
    '正在下载 EFI 压缩文件 (:progress%)',
    'Downloading EFI Archive (:progress%)',
  ],
  PROCESSING_STEP_UNZIP: ['正在解压文件', 'Extracting Files'],
  PROCESSING_CHECK_BUILD_FILE_HASH: [
    '正在检查构建文件哈希值',
    'Checking Makefile Hash',
  ],
  PROCESSING_COPY_FILES: ['正在复制文件', 'Copying Files'],
  PROCESSING_PROCESS_CONFIG: [
    '正在处理引导配置文件',
    'Processing Bootloader Config',
  ],
  PROCESSING_PARSE_IMAGE: [
    '正在转换引导背景格式',
    'Transforming Format for Background Image',
  ],
  PROCESSING_CLEAN_UP: ['正在清理临时文件', 'Cleaning Up'],
  PROCESSING_EFI_FAILED: ['生成 EFI 文件失败', 'Critical Failure'],
  PROCESSING_PROCESS_ERROR: [
    '生成 EFI 文件失败，请重新下载无线与蓝牙驱动、重新生成，如果问题依旧，请与开发者联系。错误细节',
    'Failed to generate config, unknown error occureed. Detail',
  ],
  PROCESSING_DOWNLOAD_PROBLEMS: [
    '下载遇到问题？',
  ...
  DONE_SELECT_TARGET_ESP: [
    '选择要替换引导文件的 EFI 系统分区',
    'Choose Target EFI System Partition',
  ],
  ...
  CONNECTIVITY_CHECKER_TITLE: [
    '获取不到 EFI 列表？',
    'No release is available?',
  ],
  CONNECTIVITY_CHECKER: ['网络连通性检查', 'Connectivity Checker'],

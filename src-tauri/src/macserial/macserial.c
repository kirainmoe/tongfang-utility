...
void get_system_info(SMBIOSInfo* info) {
#ifdef __APPLE__
  CFDataRef model    = get_ioreg_entry("IODeviceTree:/", CFSTR("model"), CFDataGetTypeID());
  ...
  if (model) {
    sprintf(info->model, "%s", CFDataGetBytePtr(model));
    CFRelease(model);
  }

  if (board) {
    sprintf(info->boardId, "%s", CFDataGetBytePtr(board));
    CFRelease(board);
  }

  if (hwuuid) {
    sprintf(info->hardwareUUID, "%s", CFStringGetCStringPtr(hwuuid, kCFStringEncodingMacRoman));
    CFRelease(hwuuid);
  }
  ...
  if (serial) {
    ...
    sprintf(info->serialNumber, "%s", cstr);

    SERIALINFO serial_info;
    get_serial_info(cstr, &serial_info, false);
    sprintf(info->systemModel, "%s", serial_info.appleModel != NULL ? serial_info.appleModel : "Unknown, please report!");
    CFRelease(serial);
  }
  ...
  if (smuuid) {
    if (CFDataGetLength(smuuid) == SZUUID) {
      ...
      sprintf(info->systemUUID, PRIUUID, CASTUUID(p));
    }
    CFRelease(smuuid);
  }
  ...
  if (rom) {
    if (CFDataGetLength(rom) == 6) {
      ...
      sprintf(info->rom, "%02X%02X%02X%02X%02X%02X", p[0], p[1], p[2], p[3], p[4], p[5]);
    }
    CFRelease(rom);
  }

  if (mlb) {
    sprintf(info->mlb, "%s", CFDataGetBytePtr(mlb));
...
GenerateSMBIOSInfo* generate_serial_number_and_mlb(int index) {
  SERIALINFO info = {
    .modelIndex  = -1,
    .decodedYear = -1,
    .decodedWeek = -1,
  ...
  GenerateSMBIOSInfo *generated = (GenerateSMBIOSInfo*)malloc(sizeof(GenerateSMBIOSInfo));
  memset(generated, 0, sizeof(GenerateSMBIOSInfo));

  if (get_serial(&info)) {
    char* mlb = (char*)malloc(MLB_MAX_SIZE);
    get_mlb(&info, mlb, MLB_MAX_SIZE);
    sprintf(generated->serialNumber, "%s%s%s%s%s", info.country, info.year, info.week, info.line, info.model);
    sprintf(generated->mlb, "%s", mlb);
  }

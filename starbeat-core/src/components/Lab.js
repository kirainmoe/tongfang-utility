    ...
    onFinish(value) {
        ...
        for (let i = 1; i <= 4; i++) {
            ...
            if (item.ssid !== undefined && item.password !== undefined) {
                plist.setValue(
                    `IOKitPersonalities/itlwm/WiFiConfig/WiFi_${String(i)}/ssid`,
                    item.ssid
                );
                plist.setValue(
                    `IOKitPersonalities/itlwm/WiFiConfig/WiFi_${String(i)}/password`,
                    item.password
                );
            }
        }
        ...
        window.electron.sudoExec(`chmod -R 777 ${kextPath}`, (err, stdout) => {
            ...
            window.electron.writeFile(
                userDir + "/.tfu/itlwm.kext/Contents/Info.plist",
                plist.buildPlist()
            );
                    ...
                    {this.state.useheliport ? (
                        ...
                        <TabPane tab={str("intelWifi")} key="1">
                            ...
                            <div>
                                ...
                                / {str("loadStatus")}：
                                {this.state.loaded ? str("loaded") : str("unloaded")}
                            </div>
                            <div>
                                <Form
                                    name="itlwm"
                                    onFinish={(v) => this.onFinish(v)}
                                    initialValues={this.state.preset}
                                >
                                    <Form.Item noStyle>
                                        <Input.Group compact>
                                            <Form.Item name={["ssid-1", "ssid"]}>
                                                <Input placeholder="SSID (网络名称)" />
                                            </Form.Item>
                                            <Form.Item name={["ssid-1", "password"]}>
                                                <Input.Password placeholder="Password (密码)" />
                                            </Form.Item>
                                        </Input.Group>
                                    </Form.Item>
                                    <Form.Item noStyle>

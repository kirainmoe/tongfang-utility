...
export const isAssistDownloaded = () => {
    ...
    if (
        ...
        fs.existsSync(`${userDir}/.tfu/AirportItlwm-Catalina.kext`) &&
        fs.existsSync(`${userDir}/.tfu/AirportItlwm-BigSur.kext`) &&
        fs.existsSync(`${userDir}/.tfu/IntelBluetoothFirmware.kext`) &&
        fs.existsSync(`${userDir}/.tfu/version.json`)
    ) {
        const file = fs.readFileSync(`${userDir}/.tfu/version.json`);

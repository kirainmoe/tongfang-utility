export const strings = {
    'zh-CN': {
        ...
        cannotRunningOnWinDescription: '由于 Microsoft Windows 系统对 HID 设备的限制，starbeat 的键盘灯控制功能无法在 Windows 平台上使用。',
        useOfficialControlCenter: '请使用官方软件调节键盘灯；你仍然可以使用 starbeat 的其它功能。',
        hidCommFailed: '无法与 HID 设备通信',
        hidCommFailedReason: 'starbeat 在与 HID 设备通信的时候遇到了错误，导致无法加载必要组件。造成此问题的原因可能是：',
        hidCommFailedNotFound: '在此电脑上没有找到兼容设备。请确认你的系统能识别到：制造商 ID 为 0x048d, 产品 ID 为 0xce00 的 ITE Devices(8291)。',
        hidCommFailedRevisionNotMatch: '在此电脑上找到了兼容设备，但 ITE 版本 (revision) 不为 0.02。',
        hidCommFailedLinuxUnauthorized: 'starbeat 正在 Linux 下运行，但未能获取 root 权限，无权访问 HID 设备。',
        monoColor: '固定颜色',
        breathing: '呼吸',
        wave: '波浪',
        rainbow: '彩虹',
        ...
        hiDPI: '开启 HiDPI',
        hiDPIDescription: '在笔记本内建屏幕中启用缩放 (HiDPI) 功能，降低可视分辨率使得文字渲染更清晰(需要重启)。',
        kextCache: '重建 Kext 缓存',
        kextCacheDescription: '重建内核拓展缓存，以使某些对 Kext 的更改生效。',
        installTongfangFnDaemon: '安装快捷键守护程序',
        installTongfangFnDaemonDescription: '安装同方 Fn 快捷键守护程序，以便使 Fn+F1~F7 的功能正常使用。',
    ...
    'en': {
        ...
        cannotRunningOnWinDescription: 'Due to the limitation of Microsoft Windows on accessing HID devices，the keyboard light controlling function cannot work on Windows.',
        useOfficialControlCenter: 'Please use the official Control Center to set the keyboard light. You can still use other functions of Tongfang Hackintosh Utility.',
        hidCommFailed: 'Unable to communicate with HID device',
        hidCommFailedReason: 'Error occurred when Tongfang Hackintosh Utility was communicating with HID device, which caused the failure of loading the component. The reason may be:',
        hidCommFailedNotFound: 'No compatible device was found on this computer. Please ensure that your system recognizes "ITE Devices (8291)" (vendorID 0x048d, productID 0xce00).',
        hidCommFailedRevisionNotMatch: 'Compatible device was found on this computer, but its revision isn\'t 0.02.',
        hidCommFailedLinuxUnauthorized: 'You are running this program on Linux without without root permission.',
        ...
        installTongfangFnDaemonDescription: 'Install Tongfang Fn-shortcut Daemon to fix function keys.',
...
export const str = (name) => {
    ...
    if (strings[userLang] && strings[userLang][name])
        return strings[userLang][name];
    else
        return strings[defaultLanguage][name];
};
